...
setwd("/home/leonardo/Documents/Projects/VOGss/5_GettingDomains/D_getDomains/3_finalDomains")
args <- commandArgs(trailingOnly = TRUE)
output <- read.table(file.choose(), header=F, sep="\t")
print("Table readed...")
...
parsedOutput <- rbind(parsedOutput, domain)
write.table(x=parsedOutput, file="parsed.tsv", sep="\t", row.names=F, quote=F)
